...
//                    properSize = Math.min(size,YyScreenUtils.dp2px(getContext(),72));
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		//新需求:按住停止滑动一段时间后，需要回调外部给到当前position信息
		callbackAfterAWhile(event);
	 ...
	 * 延迟一段时间后回调用户
	 */
	private class RunnableDelayCallback implements Runnable {

		@Override
		public void run() {
			if (mListener != null) {
				mListener.onProgressDelayChange(mScalesManager.getIndex());
			}
		}
	}

	private Runnable mDelayCallback = new RunnableDelayCallback();
	private static int TIMEOUT_DELAY = 500;//延迟回调时间，单位：毫秒
	private int mDelayLastPosition = 0;
	...
	private void delayCallback() {
		...
		mHandler.postDelayed(mDelayCallback, TIMEOUT_DELAY);
	}
